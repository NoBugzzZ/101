...
void Renderer::Render(const Scene &scene)
{
    ...
    Vector3f eye_pos(0);
    Vector3f leftTopPoint(-scene.width/2, scene.height/2, -scene.height / (2 * scale));
    int m = 0;
    for (int j = 0; j < scene.height; ++j)
    {
        for (int i = 0; i < scene.width; ++i)
        {
            ...
            Vector3f pixel(leftTopPoint.x + i + 0.5, leftTopPoint.y - j - 0.5, leftTopPoint.z);
            // Vector3f dir = Vector3f(x, y, -1); // Don't forget to normalize this direction!
            Vector3f dir = normalize(pixel);
