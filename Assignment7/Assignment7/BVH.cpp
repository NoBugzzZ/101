...
Intersection BVHAccel::getIntersection(BVHBuildNode *node, const Ray &ray) const
{
    ...
    {
        // std::cout<<"objet getIntersection"<<std::endl;
        return node->object->getIntersection(ray);
    }

    Intersection leftInter = getIntersection(node->left, ray);
    if (leftInter.happened && leftInter.distance < inter.distance)
    {
        inter = leftInter;
    }
    Intersection rightInter = getIntersection(node->right, ray);
    if (rightInter.happened && rightInter.distance < inter.distance)
    {
        inter = rightInter;
    }
